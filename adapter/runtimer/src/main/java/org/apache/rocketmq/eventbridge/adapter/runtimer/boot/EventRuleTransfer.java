...
public class EventRuleTransfer extends ServiceThread {
    ...
    public void run() {
        while (!stopped) {
            String runnerName = circulatorContext.takeEventBusRunnerName();
            ConnectRecord eventRecord = circulatorContext.takeEventRecord(runnerName);
            ...
            try {
                ...
                circulatorContext.offerTargetTaskQueue(runnerName, afterTransformConnect);
                // transform处理完成后将处理完成可以推送的runnerName写入队列，通知pusher
                circulatorContext.offerEventRuleRunnerNameQueue(runnerName);
